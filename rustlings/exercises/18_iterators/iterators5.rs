...
fn count_collection_iterator(collection: &[HashMap<String, Progress>], value: Progress) -> usize {
    ...
    collection.iter()             // 1. Get iterator yielding &HashMap<...>
        .flat_map(|map| map.values()) // 2. For each map, get its values iterator,
                                      //    and chain them all into one big iterator yielding &Progress
        .filter(|&val| *val == value) // 3. Keep only values matching `value`
        .count()
}
